...
class PentaBlackFlatSetup extends StatefulWidget {
  const PentaBlackFlatSetup({super.key});

  @override
  State<PentaBlackFlatSetup> createState() => _PentaBlackFlatSetupState();
}

class _PentaBlackFlatSetupState extends State<PentaBlackFlatSetup> {
  ...
  Future getPrefValue() async {
    ...
    controllers.forEach((itemName, controller) {
      String? value = prefs.getString("anchorPentaBlackFlat$itemName");
      if (value != null) {
  ...
  Map<String, TextEditingController> createTextEditingControllers() {
    ...
    pentaBlackFlatItems.forEach((itemName, _) {
            ...
            children: [
              Expanded(
                child: ListView.builder(
                  itemCount: pentaBlackFlatItems.length,
                  itemBuilder: (context, index) {
                    String itemName = pentaBlackFlatItems.keys.elementAt(index);
                    return Card(
  ...
  Future<void> storeValuesInSharedPreferences() async {
    ...
    controllers.forEach(
      (itemName, controller) {
        prefs.setString("anchorPentaBlackFlat$itemName", controller.text);
      },
    );
  }
}
