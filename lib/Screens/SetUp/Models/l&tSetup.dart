...
class LTSetup extends StatefulWidget {
  const LTSetup({super.key});

  @override
  State<LTSetup> createState() => _LTSetupState();
}

class _LTSetupState extends State<LTSetup> {
  ...
  Future getPrefValue() async {
    ...
    controllers.forEach((itemName, controller) {
      String? value = prefs.getString("l&TEngem$itemName");
      if (value != null) {
            ...
            children: [
              Expanded(
                child: ListView.builder(
                  itemCount: l$TItems.length,
                  itemBuilder: (context, index) {
                    String itemName = l$TItems.keys.elementAt(index);
                    return Card(
  ...
  Future<void> storeValuesInSharedPreferences() async {
    ...
    controllers.forEach(
      (itemName, controller) {
        prefs.setString("l&TEngem$itemName", controller.text);
      },
    );
  }
}
